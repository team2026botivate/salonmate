...
export default function Sidebar({
          ...
          <ul>
            {menuItems.map((item) => {
              ...
              if (user?.role === 'staff' && staffHidden.has(item.id)) {
                ...
                const canSee =
                  user?.permissions?.includes('all') || (perm && user?.permissions?.includes(perm));
  ...
  return (
    <motion.div
      className={`hideScrollBar fixed inset-0 z-20 flex h-screen w-full transform flex-col border-r border-gray-200 bg-white shadow-xl md:relative md:w-72 ${
