...
export const AddServiceForm = ({ onAddService }) => {
  ...
  const processCsvUpload = async () => {
    ...
    try {
      // Get store ID (assuming it's available)
      const storeId = localStorage.getItem('storeId') || '05d0bb2b-4322-4291-9151-cd30f18b39fc';
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="flex space-x-4">
          <motion.button
            ...
            whileTap={{ scale: 0.98 }}
            className={`px-6 py-3 bg-indigo-600 text-white rounded-lg font-medium hover:bg-indigo-700 transition-colors flex items-center justify-center space-x-2 ${isSubmitting ? 'opacity-75 cursor-not-allowed' : ''
              }`}
          >
            {isSubmitting ? (
              <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-white"></div>
